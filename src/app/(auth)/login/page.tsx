...
export default async function Login({ searchParams }: LoginProps) {
    ...
    async function LoginUser(formData: FormData) {
        ...
        if (password.length < 8) {
            redirect('/login?error=A senha deve ter pelo menos 8 caracteres');
        }
        ...
        // Successful login: redireciona com username
        const username = data?.user?.user_metadata?.username;
        redirect(username ? `/?username=${encodeURIComponent(username)}` : '/');
    }
    ...
    return (
        <main className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
            {/* Toast Notifications */}
            {params.error && (
                <Toast type="error" message={decodeURIComponent(params.error)} duration={6000} />
            )}
            {params.success && (
                <Toast type="success" message={decodeURIComponent(params.success)} duration={8000} />
            )}
            {params.message && (
                <Toast type="info" message={decodeURIComponent(params.message)} duration={6000} />
            )}
            ...
            <div className="max-w-md w-full space-y-8">
                ...
                <div className="text-center">
                    ...
                    <h2 className="text-3xl font-bold text-blue-900 mb-2">Faça Login</h2>
                    <p className="text-blue-700 text-sm">Bem-vindo de volta! Entre na sua conta</p>
                </div>
